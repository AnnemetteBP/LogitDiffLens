...
def gpt_blocks_input_locator(model: nn.Module):
    """
...
def gpt_final_layernorm_locator(model: nn.Module):
...
def _locate_special_modules(model):
    if not hasattr(model, "_blocks_input_getter"):
        model._blocks_input_getter = gpt_blocks_input_locator(model)

    if not hasattr(model, "_ln_f_getter"):
        model._ln_f_getter = gpt_final_layernorm_locator(model)
...
def make_gpt_decoder(model, decoder_layer_names=['final_layernorm', 'lm_head']):
...
def make_gpt_lens_hooks(
    ...
    model._lens_decoder = make_gpt_decoder(model, decoder_layer_names)
    ...
    def _make_record_logits_hook(name):
        ...
        def _record_logits_hook(module, input, output) -> None:
            ...
            # Store as PyTorch tensor on device, no .numpy()
...
def clear_gpt_lens_hooks(model):
